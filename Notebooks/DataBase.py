...
class DataBase:
    def __init__ (self, link, language = "en"):
        try:
            self.__data = pd.read_csv(link)                 #Aqui é onde esta os dados principais
            self.__entropia = 0                             #Aqui estará a entropia do target
            self.__header = list(self.__data.keys())          #Aqui estará a lista de cabeçarios
            self.__translation = []                         #Aqui estará a lista de dicionario de traduçoes
            for i in range(len(self.__header)):
                self.__translation.append(list(self.__data[self.__header[i]].value_counts().sort_index().keys())) #prenche a traducao com os elementos contidos na coluna
            try:
    ...
    @translation.setter
    def translation(self, tra):
        self.__translation = tra
    #=============================================================================================
    ...
    def dropColumn(self, ind):
        ...
        clone = self.__data
        clonec = []
        clonec.extend(self.__translation)
        
        k = 0
        tamt = len(self.__translation)
        temp = 0
        for i in ind:
            clone = clone.drop(self.__data.keys()[i], axis = 1)
    ...
    def showGraphic(self, i):
        ...
        a, b, c = ax.pie(self.__data[self.__header[i]].value_counts().sort_index().values, 
                        labels = self.__data[self.__header[i]].value_counts().sort_index().index, 
                        autopct = '%1.1f%%', labeldistance = 1.2, pctdistance = 0.8)
        
        alt = 0
        if len(self.__translation[i]) > 6:
            alt = -0.6
        else:
            alt = -0.3
        ax.legend(self.__translation[i], title= self.__Strings.find("DataBase/Legend/string[@name='s1']").text, loc= 8, bbox_to_anchor=(0, alt, 1, 0), ncol = 3)
        plot.setp(c, size=8, weight="bold")
        ax.set_title(self.__header[i])
        plot.show()
        
    def showGraphict(self, i, j):
        fig, ax = plot.subplots(ncols = 2, figsize=(16, 4), subplot_kw=dict(aspect="equal"))
        
        a, b, c = ax[0].pie(self.__data[self.__header[i]].value_counts().sort_index().values, 
                        labels = self.__data[self.__header[i]].value_counts().sort_index().index, 
                        autopct = '%1.1f%%', labeldistance = 1.2, pctdistance = 0.8)
        
        d, f, g = ax[1].pie(self.__data[self.__header[j]].value_counts().sort_index().values, 
                        labels = self.__data[self.__header[j]].value_counts().sort_index().index, 
                        autopct = '%1.1f%%', labeldistance = 1.2, pctdistance = 0.8)
        
        ax[0].legend(self.__translation[i], title = (self.__header[i]) ,  loc= 8 , bbox_to_anchor=(0, -0.3, 1, 0), ncol = 2)
        ax[1].legend(self.__translation[j], title = (self.__header[j]) ,  loc= 8 , bbox_to_anchor=(0, -0.3, 1, 0), ncol = 2)
        
        plot.setp(c, size=8, weight="bold")
        plot.setp(g, size=8, weight="bold")
        
        ax[0].set_title(self.__header[i])
        ax[1].set_title(self.__header[j])
    ...
    def showGraphicCompare(self, ind, list2, lab2, leg2):
        ...
        d, f, g = ax[1].pie(self.__data[self.__header[ind]].value_counts().sort_index().values, 
                        labels = self.__data[self.__header[ind]].value_counts().sort_index().index, 
                        autopct = '%1.1f%%', labeldistance = 1.2, pctdistance = 0.8)
        alt1 = 0
        alt2 = 0

        if len(self.__translation[ind]) > 6:
            alt1 = -0.6
        else:
            alt1 = -0.3
        ...
        ax[1].legend(self.__translation[ind], title = self.__Strings.find("DataBase/Legend/string[@name='s1']").text ,  loc= 8 , bbox_to_anchor=(0, alt1, 1, 0), ncol = 2)
        
        
        plot.setp(c, size=8, weight="bold")
        plot.setp(g, size=8, weight="bold")
        
        ax[0].set_title(self.__header[ind]) 
        ax[1].set_title(self.__header[ind])

        plot.show()
    
    def saveCsv(self, nome):
        try:
            self.__data.to_csv(nome , sep = ',', index=False, encoding = 'utf-8', quotechar = '"', line_terminator = '\r\n')
        except:
            print(self.__Strings.find("DataBase/saveCsv/string[@name='s1']").text)
            
    def relationshipTable(self, ncoluna):
        tabel = {}
        l1 = list(self.__data[self.__header[0]].values)
        l2 = list(self.__data[self.__header[ncoluna]].values)
        
        qtl1 = list(self.__data[self.__header[0]].value_counts().sort_index().keys())
        qtl2 = list(self.__data[self.__header[ncoluna]].value_counts().sort_index().keys())
    ...
    def relationshipTablein(self, xcoluna, ncoluna):
        tabela = {}
        l1 = list(self.__data[self.__header[xcoluna]].values)
        l2 = list(self.__data[self.__header[ncoluna]].values)
        
        qtl1 = list(self.__data[self.__header[xcoluna]].value_counts().sort_index().keys())
        qtl2 = list(self.__data[self.__header[ncoluna]].value_counts().sort_index().keys())
    ...
    def setEntropia(self, ncoluna):
        ...
        self.__entropia = self.entropiaS(self.__data[self.__header[ncoluna]].value_counts().values, len(self.__data))
    ...
    def pdRelation(self, coluna, linha):
        ...
        dat.index   = self.__translation[linha]
        dat.columns = self.__translation[coluna]
